package org.zackratos.attemptmvp.updateApk;
...
/**
 * Created by leaf on 2017/11/21.
 * App更新下载自动安装
 * 不需要改动整个代码，只需要在apk存放路径的时候改成自己的存放路径就可以了
 */
...
public class DownLoadService extends Service {
    ...
    private String url;
    private String DOWNLOADPATH = "/ps/apk/";// 需要修改的地方
    ...
    private void initDownManager() {
        ...
        down.setDestinationInExternalPublicDir(DOWNLOADPATH, "ps.apk");// 第一个参数为下载下来的Apk放的位置的路径（注意是文件夹），第二个参数为文件名称 // 需要修改的地方
        down.setTitle("ps");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        String path = Environment.getExternalStorageDirectory().getAbsolutePath() + DOWNLOADPATH + "ps.apk";// 内置存储卡路径 // 需要修改的地方
    ...
    public void openFile(File var0, Context var1) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri uriForFile = FileProvider.getUriForFile(var1, "org.zackratos.attemptmvp.provider", var0);// 第一个参数数为：包名+“.provider” // 需要修改的地方
